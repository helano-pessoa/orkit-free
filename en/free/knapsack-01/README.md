...
| Exact            | OR-Tools CP-SAT    | `exact/model_ortools.py`      |
...
├── metaheuristics/
│   ├── sa.py                ← Simulated Annealing (bit-flip + greedy repair)
│   ├── grasp.py             ← GRASP (RCL by profit/weight ratio + local search)
│   └── ga.py                ← Genetic Algorithm (binary chromosome + elitism)
├── notebooks/
